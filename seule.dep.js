...
class Seule {
    constructor(selector, root, style, external) {
        ...
        if(root && typeof root !== 'object') {
            this.render();
            this.children = child;
            console.log("%cDon't try anything cute,\n" +
                "%cDom is Protected! 😡",
        ...
        if (options) {
            ...
            return this.Each(function () {
                ...
                let element = this,
                    ...
                    if(options.loop || options.direction)
                        if(options.delay) delay = (parseFloat(options.delay.replace(/s/g, "")))*1000
    ...
    Component(name, options) {
        ...
        if(options.data) if(!options.data.length){
            options.obj = []
    ...
    static PRINT(options) {
        ...
        if(options.style) frameDoc.document.write(
            '<link href="' + options.style + '.css" rel="stylesheet" type="text/css" />'
    ...
    static PDO(data, options) {
        ...
        let obj = [...data],
            ...
            result.group = (column, obj) => {
                ...
                let da = [];
                let group = [
                    ...new Set(obj.reduce((r, a) => r.concat(a[column]), []))
                ];
                Seule.LOOP({
                    data: group,
            ...
            result.removeDpl = (column, obj) => {
                ...
                if (column) {
                    ...
                    return obj.filter(
                        (data) =>
                            !lookup.has(data[column]) &&
                            lookup.add(data[column])
                    );
                }
        ...
        if(options){
            switch (options.execute) {
                ...
                case "alter":
                    if (options.nest) result.alter(options.action, options.columns, result.child);
                    else result.alter(options.action, options.columns, result.res);
                    break;

                case "group by":
                    if (options.nest) result = result.group(options.columns, result.child);
                ...
                case "remove duplicates":
                    if (options.nest) result = result.removeDpl(options.columns || false, result.child);
                    else result = result.removeDpl(options.columns || false, result.res);
                    break;

                case "order by":
                    if (options.nest) result = result.sort(options.columns || false, result.child);
                    ...
                    class Example extends HTMLElement {
                        constructor() {
                            ...
                            if (options.mode !== "closed") {
                                els = options.element;
                                els.Select = (selector)=> new Seule(shadow.querySelectorAll(selector))
                            }
                            else els.Select = () =>
                                console.log(
    ...
    static GETATTR(element, attr, handler, ifr){
        let attrs = element.querySelectorAll('*'),
            ...
            action = {},
            obj,
            extra = 'copy text val show hide visible opacity width height attr style classes anime css',
            ...
            for (let a of e.getAttributeNames()) {
                if(a.includes('@')){
                    let val = e.getAttribute(a),
                        capitalStr = a.replace('@', '').replace(/\w/, c => c.toUpperCase());
                    ...
                    if(val.includes('($')) {
                        ...
                        obj = obj
                            .replace(/[~']/g, '"')
                            .replace(/[~`]/g, '"');
                        if(allowd.includes(capitalStr.toLowerCase())) obj = val[1].slice(0, -1)
                ...
                let actions = elements[key].replace('undefined;', '').split(';'),
                    ...
                    handler = ()=>{
                        for (let act of actions){
                            let a = act.split('$')
                            if(a[0].trimStart() !== 'Find') ex(JSON.parse(a[1]), a[0].trimStart())
                        }
                    }
