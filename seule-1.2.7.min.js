(function(window,undefined){var domReadyStack=[];function handleDOMReady(fn){return document.readyState==='complete'?fn.call(document):domReadyStack.push(fn)}
    ...
    !function(t,e){"use strict";var n=null,o="ontouchstart" in t||navigator.MaxTouchPoints>0||navigator.msMaxTouchPoints>0,u=o?"touchstart":"mousedown",s=o?"touchcancel":"mouseout",i=o?"touchend":"mouseup",c=o?"touchmove":"mousemove";"initCustomEvent" in e.createEvent("CustomEvent")&&(t.CustomEvent=function(t,n){n=n||{bubbles:!1,cancelable:!1,detail:void 0};var o=e.createEvent("CustomEvent");return o.initCustomEvent(t,n.bubbles,n.cancelable,n.detail),o},t.CustomEvent.prototype=t.Event.prototype),e.addEventListener(u,function(t){var e=t.target,o=parseInt(e.getAttribute("s-hold-time")||"1500",10);n=setTimeout(function(){this.dispatchEvent(new CustomEvent("long-press",{bubbles:!0,cancelable:!0})),clearTimeout(n),console&&console.log&&console.log()}.bind(e),o)}),e.addEventListener(i,function(t){clearTimeout(n)}),e.addEventListener(s,function(t){clearTimeout(n)}),e.addEventListener(c,function(t){clearTimeout(n)})}(this,document);Seule.fn=Seule.prototype;Seule.fn.each=function(callback){for(var i=0;i<this.length;i++){callback.call(this[i],this,i)}
        ...
        if(module.indexOf("cryptographic")>-1){this.call('cryptographic')}
        ...
        return this};Seule.fn.objectToUrlQuery=function(obj){var s=Object.keys(obj).map(function(key){return key+"="+obj[key]}).join("&");return this.length&&("'"+s+"'").slice(1).slice(0,-1)};Seule.fn.storage=function(nom,objet){return this.length&&new Promise(function(resolve,reject){window.localStorage.setItem(nom,JSON.stringify(objet));resolve(nom+" was successfully created!");reject("Can not create "+nom)})};Seule.fn.getStorage=function(nom){return this.length&&JSON.parse(window.localStorage.getItem(nom))};Seule.fn.storageDelete=function(nom){return this.length&&new Promise(function(resolve,reject){window.localStorage.removeItem(nom);resolve(nom+" was deleted!");reject("Unable to delete"+nom)})};Seule.fn.storageClear=function(){return this.length&&new Promise(function(resolve,reject){window.localStorage.clear();resolve("Local Storage is clear");reject("Unable to reset the Local Stocakge")})};Seule.fn.select=function(from,object,value){var param=value.split(" "),colName=param[1];if(param.length==4){if(param[2]==">"){this.promes=object.filter(function(d){return d[colName].toLowerCase()>param[3].toLowerCase()})}else if(param[2]=="<"){this.promes=object.filter(function(d){return d[colName].toLowerCase()<param[3].toLowerCase()})}else if(param[2]=="EQUAL"||param[2]=="LIKE"||param[2]=="="){this.promes=object.filter(function(d){return d[colName].toLowerCase()==param[3].toLowerCase()})}
        return this&&this.promes}
    else if(param.length==6){if(param[1]=="Between"){this.promes=object.filter(function(d){return d[colName].toLowerCase()>=param[3].toLowerCase()&&d[colName].toLowerCase()<=param[5].toLowerCase()})}
        return this&&this.promes}
    else{this.promes=object.filter(function(x){for(var i in x){if(x[i].toLowerCase().indexOf(value.toLowerCase())>-1)return x}});return this&&this.promes}};Seule.fn.multipleSelect=function(from,json,query,values){var column=query.split(" "),data1=values,obj=[];for(i=0;i<json.length;i++){obj[i]=json[i]}
        var i=obj.length;while(i--){if(data1.indexOf(obj[i][column[1]])==-1){obj.splice(i,1)}}
        return obj};Seule.fn.delete=function(from,json,query,values){var column=query.split(" ");var data1=values;var i=json.length;while(i--){if(data1.indexOf(json[i][column[1]])!=-1){json.splice(i,1)}}
        ...
        return this};Seule.fn.mSelect=function(selector){return document.querySelectorAll(this.parrent+" "+selector)};Seule.fn.iniT=function(attrib){var boite=this.mSelect(" ["+attrib+"]"),resultType=[],resultEvent=[];for(var i=0;i<boite.length;i++){var valeur=boite[i].getAttribute(attrib);var option=valeur.split("{");var event=option[0];var type=option[1].slice(0,-1);resultType.push(type);resultEvent.push(event)}
