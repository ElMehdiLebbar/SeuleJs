...
class Seule {
        ...
        this.ren = (con) => {
            ...
            for (let item of keys) {
                while (content.includes("{{" + item + "}}"))
                    content = content.replace("{{" + item + "}}", this.data[item].replace(/<[^>]*>/g, ''));
            }
    ...
    static Scroll() {
        ...
        sc.onBottom = (handler, before= 0) =>{
            window.onscroll = ev => {
                if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight-before) handler(this)
            };
            return this
        };
    ...
    static Print(options) {
        ...
        options.style && frameDoc.document.write(
            '<link href="' +
            options.style +
