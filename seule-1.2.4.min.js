(function(window,undefined){var domReadyStack=[];function handleDOMReady(fn){return document.readyState==='complete'?fn.call(document):domReadyStack.push(fn)}
    document.addEventListener('DOMContentLoaded',function onDOMReady(){document.removeEventListener('DOMContentLoaded',onDOMReady);while(domReadyStack.length){domReadyStack.shift().call(document)}});function Seule(selector,child){if(!(this instanceof Seule)){if(child==null){return new Seule(selector)}else{return new Seule(selector,child)}}
        ...
        this.length=0;this.nodes=[];this.parrent=selector;if(child!=null){selector=child}
    ...
    !function(t,e){"use strict";var n=null,o="ontouchstart" in t||navigator.MaxTouchPoints>0||navigator.msMaxTouchPoints>0,u=o?"touchstart":"mousedown",s=o?"touchcancel":"mouseout",i=o?"touchend":"mouseup",c=o?"touchmove":"mousemove";"initCustomEvent" in e.createEvent("CustomEvent")&&(t.CustomEvent=function(t,n){n=n||{bubbles:!1,cancelable:!1,detail:void 0};var o=e.createEvent("CustomEvent");return o.initCustomEvent(t,n.bubbles,n.cancelable,n.detail),o},t.CustomEvent.prototype=t.Event.prototype),e.addEventListener(u,function(t){var e=t.target,o=parseInt(e.getAttribute("s-hold-time")||"1500",10);n=setTimeout(function(){this.dispatchEvent(new CustomEvent("long-press",{bubbles:!0,cancelable:!0})),clearTimeout(n),console&&console.log&&console.log()}.bind(e),o)}),e.addEventListener(i,function(t){clearTimeout(n)}),e.addEventListener(s,function(t){clearTimeout(n)}),e.addEventListener(c,function(t){clearTimeout(n)})}(this,document);Seule.fn=Seule.prototype;Seule.fn.each=function(callback){for(var i=0;i<this.length;i++){callback.call(this[i],this,i)}
        ...
        if(obj.times!=null){for(var i=0;i<obj.times;i++){obj.handler()}}else{[].forEach.call(mir,obj.handler)}};Seule.fn.call=function(module){let lastTree=module.substr(module.length-3);if(lastTree!='.js'){module="seule."+module+".min.js"}
        ...
        break;default:this.call(module)}};Seule.fn.objectToUrlQuery=function(obj){var s=Object.keys(obj).map(function(key){return key+"="+obj[key]}).join("&");return("'"+s+"'").slice(1).slice(0,-1)};Seule.fn.storage=function(nom,objet){return new Promise(function(resolve,reject){window.localStorage.setItem(nom,JSON.stringify(objet));resolve(nom+" was successfully created!");reject("Can not create "+nom)})};Seule.fn.getStorage=function(nom){return JSON.parse(window.localStorage.getItem(nom))};Seule.fn.storageDelete=function(nom){return new Promise(function(resolve,reject){window.localStorage.removeItem(nom);resolve(nom+" was deleted!");reject("Unable to delete"+nom)})};Seule.fn.storageClear=function(){return new Promise(function(resolve,reject){window.localStorage.clear();resolve("Local Storage is clear");reject("Unable to reset the Local Stocakge")})};Seule.fn.select=function(from,object,value){var param=value.split(" "),colName=param[1],result,data;if(param.length==4){if(param[2]==">"){result=object.filter(function(d){return d[colName].toLowerCase()>param[3].toLowerCase()})}else if(param[2]=="<"){result=object.filter(function(d){return d[colName].toLowerCase()<param[3].toLowerCase()})}else if(param[2]=="EQUAL"||param[2]=="LIKE"||param[2]=="="){result=object.filter(function(d){return d[colName].toLowerCase()==param[3].toLowerCase()})}
        return result}
    else if(param.length==6){if(param[1]=="Between"){data=object.filter(function(d){return d[colName].toLowerCase()>=param[3].toLowerCase()&&d[colName].toLowerCase()<=param[5].toLowerCase()})}
        return data}
    else{return object.filter(function(x){for(var i in x){if(x[i].toLowerCase().indexOf(value.toLowerCase())>-1)return x}})}};Seule.fn.multipleSelect=function(from,json,query,values){var column=query.split(" "),data1=values,obj=[];for(i=0;i<json.length;i++){obj[i]=json[i]}
        ...
        return json};Seule.fn.insertInTo=function(json,value,element){return new Promise(function(resolve,reject){json.push(element);resolve(json);reject("Operation was canceled")})};Seule.fn.update=function(json,query){var params=query.split(" ");for(var i=0;i<json.length;i++){if(json[i][params[5]]===params[7]){json[i][params[1]]=params[3]}}};Seule.fn.parent=function(){return new Seule(this[0].parentElement)};Seule.fn.child=function(index){return new Seule(this[0].children[index])};Seule.fn.first=function(){return Seule(this[0].children[0])};Seule.fn.last=function(){return Seule(this[0].children[this.length+1])};Seule.fn.filter=function(attribute){var result=[];var newArray=Array.from(this[0].children).filter(function(item){result.push(item.hasAttribute(attribute))});for(var i=0;i<result.length;i++){if(result[i]==!1){this[0].children[i].style.display="none"}}};Seule.fn.width=function(value){if(value){return this.each(function(){this.style.width=value})}
