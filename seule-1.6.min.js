(function(window,undefined){var domReadyStack=[],CryptoJS=CryptoJS||function(t,e){var r={},i=r.lib={},n=function(){},o=i.Base={extend:function(t){n.prototype=this;var e=new n;return t&&e.mixIn(t),e.hasOwnProperty("init")||(e.init=function(){e.$super.init.apply(this,arguments)}),e.init.prototype=e,e.$super=this,e},create:function(){var t=this.extend();return t.init.apply(t,arguments),t},init:function(){},mixIn:function(t){for(var e in t)t.hasOwnProperty(e)&&(this[e]=t[e]);t.hasOwnProperty("toString")&&(this.toString=t.toString)},clone:function(){return this.init.prototype.extend(this)}},s=i.WordArray=o.extend({init:function(t,e){t=this.words=t||[],this.sigBytes=null!=e?e:4*t.length},toString:function(t){return(t||a).stringify(this)},concat:function(t){var e=this.words,r=t.words,i=this.sigBytes;if(t=t.sigBytes,this.clamp(),i%4)
    ...
    !function(t,e){"use strict";var n=null,o="ontouchstart" in t||navigator.MaxTouchPoints>0||navigator.msMaxTouchPoints>0,u=o?"touchstart":"mousedown",s=o?"touchcancel":"mouseout",i=o?"touchend":"mouseup",c=o?"touchmove":"mousemove";"initCustomEvent" in e.createEvent("CustomEvent")&&(t.CustomEvent=function(t,n){n=n||{bubbles:!1,cancelable:!1,detail:void 0};var o=e.createEvent("CustomEvent");return o.initCustomEvent(t,n.bubbles,n.cancelable,n.detail),o},t.CustomEvent.prototype=t.Event.prototype),e.addEventListener(u,function(t){var e=t.target,o=parseInt(e.getAttribute("s-hold-time")||"1500",10);n=setTimeout(function(){this.dispatchEvent(new CustomEvent("long-press",{bubbles:!0,cancelable:!0})),clearTimeout(n),console&&console.log&&console.log()}.bind(e),o)}),e.addEventListener(i,function(t){clearTimeout(n)}),e.addEventListener(s,function(t){clearTimeout(n)}),e.addEventListener(c,function(t){clearTimeout(n)})}(this,document);function sortByProperty(property){return function(a,b){if(a[property]>b[property])
        ...
        return-1;return 0}}
    function scrollThis(element,position){if(element==null){element=window}
        element.scrollTo({top:position,behavior:'smooth'})}
    Seule.fn=Seule.prototype;Seule.fn.each=function(callback){for(var i=0;i<this.length;i++){callback.call(this[i],this,i)}
        ...
        if(module.indexOf("cryptography")>-1){this.callCrypto()}
        ...
        return this};Seule.fn.objectToStyle=function(obj){return this.objectToUrlQuery(obj).replace(/&/g,";").replace(/=/g,":")};Seule.fn.objectToUrlQuery=function(obj){var s=Object.keys(obj).map(function(key){return key+"="+obj[key]}).join("&");return this.length&&("'"+s+"'").slice(1).slice(0,-1)};Seule.fn.storage=function(name,object){window.localStorage.setItem(name,JSON.stringify(object));return this};Seule.fn.getStorage=function(name){return this.length&&JSON.parse(window.localStorage.getItem(name))};Seule.fn.storageDelete=function(name){window.localStorage.removeItem(name);return this};Seule.fn.storageClear=function(){window.localStorage.clear()};Seule.fn.select=function(from,object,query,val,orderBy){var param=query.split(" "),cols=from.split(" "),colName=param[1],order,result,rows=[],item;if(param.length==3){if(param[2]==">"){this.promes=object.filter(function(d){item=parseInt(d[colName]);return item>parseInt(val)})}
        ...
        return c})};Seule.fn.parent=function(){return new Seule(this[0].parentElement)};Seule.fn.children=function(index){if(index!=null){return new Seule(this[0].children[index])}
        return new Seule(this[0].querySelectorAll(":scope > *"))};Seule.fn.find=function(element){return new Seule(this.parrent+' '+element)};Seule.fn.first=function(){return Seule(this[0])};Seule.fn.last=function(){return Seule(this[this.length])};Seule.fn.filter=function(attribute){var result=[];var newArray=Array.from(this[0].children).filter(function(item){result.push(item.hasAttribute('data-'+attribute))});for(var i=0;i<result.length;i++){if(result[i]==!1){this[0].children[i].style.display="none"}}
        return this};Seule.fn.decrypt=function(secret,data){var decrypted=CryptoJS.AES.decrypt(data,secret);if(typeof data==='object'){return JSON.parse(decrypted.toString(CryptoJS.enc.Utf8))}
        return decrypted.toString(CryptoJS.enc.Utf8)};Seule.fn.encrypt=function(secret,data){if(typeof data==='object'){data=JSON.stringify(data)}
        var encrypted=CryptoJS.AES.encrypt(data,secret);return encrypted.toString()};Seule.fn.mSelect=function(selector){return document.querySelectorAll(this.parrent+" "+selector)};Seule.fn.iniT=function(attrib){var boite=this.mSelect(" ["+attrib+"]"),resultType=[],resultEvent=[];for(var i=0;i<boite.length;i++){var valeur=boite[i].getAttribute(attrib);var option=valeur.split("{");var event=option[0];var type=option[1].slice(0,-1);resultType.push(type);resultEvent.push(event)}
        ...
        return this};Seule.fn.hotKey=function(keys,handler){var selectorBox=document.querySelector(this.parrent);Mousetrap(selectorBox).bind(keys,function(e,combo){handler()});return this};Seule.fn.hotKeyGlobal=function(keys,handler){Mousetrap.bindGlobal(keys,function(){handler()});return this};Seule.fn.onScroll=function(handler){return this.each(function(){if(this.parrent=='html'){window.onscroll=function(){handler()}}else{this.addEventListener("scroll",handler,!1)}})};Seule.fn.scroll=function(position){return this.each(function(){var scroll=new SmoothScroll();if(position){scroll.animateScroll(position);return this}
        scroll.animateScroll(this)})};Seule.fn.scrollTop=function(){scrollThis(this[0],0);return this};Seule.fn.scrollBottom=function(){scrollThis(this[0],document.body.scrollHeight);return this};Seule.fn.scrollPosition=function(axe){var element=this[0];if(this.parrent==null||this.parrent==='html'){element=window}
        if(axe=="horizontally"){return this.length&&element.pageXOffset||element.scrollLeft}else{return this.length&&element.pageYOffset||element.scrollTop}};Seule.fn.width=function(value){if(value){return this.each(function(){this.style.width=value})}
