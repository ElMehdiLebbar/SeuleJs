(function(window,undefined){var domReadyStack=[];function handleDOMReady(fn){return document.readyState==='complete'?fn.call(document):domReadyStack.push(fn)}
    ...
    var CryptoJS=CryptoJS||function(t,e){var r={},i=r.lib={},n=function(){},o=i.Base={extend:function(t){n.prototype=this;var e=new n;return t&&e.mixIn(t),e.hasOwnProperty("init")||(e.init=function(){e.$super.init.apply(this,arguments)}),e.init.prototype=e,e.$super=this,e},create:function(){var t=this.extend();return t.init.apply(t,arguments),t},init:function(){},mixIn:function(t){for(var e in t)t.hasOwnProperty(e)&&(this[e]=t[e]);t.hasOwnProperty("toString")&&(this.toString=t.toString)},clone:function(){return this.init.prototype.extend(this)}},s=i.WordArray=o.extend({init:function(t,e){t=this.words=t||[],this.sigBytes=null!=e?e:4*t.length},toString:function(t){return(t||a).stringify(this)},concat:function(t){var e=this.words,r=t.words,i=this.sigBytes;if(t=t.sigBytes,this.clamp(),i%4)
    ...
    else if(param[2].toLowerCase()=="like"||param[2]=="="){this.promes=object.filter(function(d){item=JSON.stringify(d[colName]);return item.toLowerCase()==val.toLowerCase()})}
        if(orderBy!=null){order=orderBy.split(" ");result=this.promes.sort(sortByProperty(order[2]))}else{result=this.promes}
        if(cols[1]!=null){for(var it in result){var a=result[it][cols[0]],b=cols[0],jsonObj='{"'+b+'": "'+a+'"}';rows.push(JSON.parse(jsonObj))}
            result=rows}
        return this&&result}
        ...
        return this&&result}};Seule.fn.not=function(property,from,object,query,val,orderBy){var objitem=new this.select(from,object,query,val,orderBy);return objitem.map(function(o){var c=Object.assign({},o);for(var j=0;j<property.length;j++){delete c[property[j]]}
        ...
        return this&&result};Seule.fn.where=function(values,query,json){var item;return json.filter(function(x){for(let i in x){item=JSON.stringify(x[i]);if(item.indexOf(values)>-1)return x}})};Seule.fn.delete=function(from,json,query,values){var column=query.split(" ");var data1=values;var i=json.length;while(i--){if(data1.toString().toLowerCase().indexOf(json[i][column[1]].toString().toLowerCase())!=-1){json.splice(i,1)}}
        return this};Seule.fn.insert=function(into,json,value,element){json.push(element);return json};Seule.fn.update=function(json,set,value,where,query){var params=set.split(" "),item=where.split(" ");for(var i=0;i<json.length;i++){if(json[i][item[1]].toString().toLowerCase()==query.toString().toLowerCase()){json[i][params[1]]=value}}
        return json};Seule.fn.alter=function(json,query,columns){return json.map(function(o){var c=o;if(columns==null){var item=query.split(" ");c[item[4]]=c[item[2]];delete c[item[2]]}
        if(query.toLowerCase()==='drop'){for(var x=0;x<columns.length;x++){delete c[columns[x]]}}
        if(query.toLowerCase()==='drop'){for(var j=0;j<columns.length;j++){delete c[columns[j]]}}
